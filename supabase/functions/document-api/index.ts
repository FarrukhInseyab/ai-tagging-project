    ...
    if (path === "generate-document" && req.method === "POST") {
      ...
      if (outputFormat === "docx") {
        ...
        try {
          ...
          const docxFilename = `${baseFilename}.docx`;
          const docxStoragePath = await uploadGeneratedDocument(docxBuffer,  user ? user.id : null, generationRecord.id, docxFilename, "application/vnd.openxmlformats-officedocument.wordprocessingml.document");
          // Convert DOCX to PDF using the appropriate method based on user preferences
          const pdfBuffer = await convertDocxToPdf(docxBuffer, docxFilename,  user ? user.id : null);
          // Upload PDF to storage
          const pdfStoragePath = await uploadGeneratedDocument(pdfBuffer,  user ? user.id : null, generationRecord.id, outputFilename, "application/pdf");
          // Update generation record
          await updateGenerationRecord(generationRecord.id, [
            docxStoragePath,
            pdfStoragePath
